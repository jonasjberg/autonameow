...
done < <(find "${AUTONAMEOW_SAMPLEFILES_DIRPATH}/configs" -maxdepth 1 -xdev -type f -name 'bad_0*.yaml' -print0 | sort -z)
...
"$AUTONAMEOW_RUNNER" --quiet --dry-run --batch --config-path "$TEMPLATED_DEFAULT_CONFIG" -- "$AUTONAMEOW_SAMPLEFILES_DIRPATH" &>/dev/null
