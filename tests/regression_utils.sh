...
AUTONAMEOW_REGRESSIONTESTS_DIR="$( ( cd "$AUTONAMEOW_ROOT_DIR" && realpath -e -- "test_files/regression" ) )"
if [ ! -d "$AUTONAMEOW_REGRESSIONTESTS_DIR" ]
then
    echo "Not a directory: \"${AUTONAMEOW_REGRESSIONTESTS_DIR}\" .. Aborting" >&2
    exit 1
else
    export AUTONAMEOW_REGRESSIONTESTS_DIR
fi
...
# file and exports the the log file path as 'AUTONAMEOW_REGRESSION_LOG'.
initialize_logging()
{
    ...
    AUTONAMEOW_REGRESSION_LOG="${AUTONAMEOW_TESTRESULTS_DIR}/regression_log_${AUTONAMEOW_TEST_TIMESTAMP}.raw"
    export AUTONAMEOW_REGRESSION_LOG

    logmsg "Logging to file: \"${AUTONAMEOW_REGRESSION_LOG}\""
}

# Print message to stdout and append message to AUTONAMEOW_REGRESSION_LOG.
...
# which case AUTONAMEOW_REGRESSION_LOG will be undefined and the 'tee' call is
# skipped. In this case no log file is written do disk.
logmsg()
{
    local _timestamp="$(date "+%Y-%m-%d %H:%M:%S")"
    printf "%s %s\n" "$_timestamp" "$*" |
    ( [ ! -z "${AUTONAMEOW_REGRESSION_LOG:-}" ] && tee -a "$AUTONAMEOW_REGRESSION_LOG" || cat )
}
...
# Converts the regression test log file to HTML using executable 'aha' if
# available.  Executed at the end of a test run by 'regression_runner.sh'.
convert_raw_log_to_html()
{
    ...
    if [ -z "${AUTONAMEOW_REGRESSION_LOG:-}" ] \
    || [ ! -f "$AUTONAMEOW_REGRESSION_LOG" ]
    then
        echo "Logging has not been initialized. Aborting .." 1>&2
        return 1
    fi

    _html_regression_log="${AUTONAMEOW_REGRESSION_LOG%.*}.html"
    _html_title="autonameow regression Test Log ${AUTONAMEOW_TEST_TIMESTAMP}"

    if aha --title "$_html_title" \
        < "$AUTONAMEOW_REGRESSION_LOG" | sed 's///g' > "$_html_regression_log"
    then
        if [ -s "$_html_regression_log" ]
        then
            logmsg "Wrote regression test log HTML file: \"${_html_regression_log}\""
            rm -- "$AUTONAMEOW_REGRESSION_LOG"

            # Write log file name to temporary file, used by other scripts.
            set +o noclobber
            echo "${_html_regression_log}" > "${AUTONAMEOW_TESTRESULTS_DIR}/.regressionlog.toreport"
            set -o noclobber
        fi
    else
