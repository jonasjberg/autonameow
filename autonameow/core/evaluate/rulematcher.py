...
class RuleMatcher(object):
    ...
    def start(self):
        ...
        ok_rules = self._examine_rules(rules_to_examine)
        if len(ok_rules) == 0:
    ...
    def _evaluate_rule(self, file_rule):
        ...
        if file_rule.exact_match:
            for condition in file_rule.conditions:
                ...
                if not eval_condition(condition, self.file, self.analysis_data):
        ...
        # Rule was not completely discarded but could have failed all tests.
        return True
